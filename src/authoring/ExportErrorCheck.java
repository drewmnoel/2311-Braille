...
/**
 * Class to handle simple error checking in a scenario file before it is exported
 * Checks for things like missing jump tags, or calling buttons without 
 * 
 * @author Jon
 *
 */
public class ExportErrorCheck {
	...
	/**
	 * Method to check whether a scenario has jump events to jump tags that don't exist
	 * 
	 * @param commands list of commands that make up the scenario
	 * @return true if every jump event has a jump tag
	 * 			false otherwise
	 */
	public static boolean checkJumpTags(List<PlayerCommand> commands) {
		...
		for (PlayerCommand pc : commands) {
			...
			//if a skip tag command, increase the value of tag n tagTracker by 1
		...
		//if there are any tags in tagTracker with value 0, it means there was a jump without a matching tag
		return !tagTracker.containsValue(0);
	}
	
	/**
	 * Method to check whether a scenario calls a button that hasn't been initialized
	 * 
	 * @param commands list of commands that make up the scenario
	 * @param numButtons number of initialized buttons
	 * @return true if every button call was to a valid button
	 * 			false otherwise
	 */
	public static boolean checkButtonNumber(List<PlayerCommand> commands, int numButtons) {
		boolean errorFree = true;
		
		//iterate over command to find RepeatButtonCommands and SkipButtonCommands
		for (PlayerCommand pc : commands) {
	...
	/**
	 * Method to check whether a scenario tries to call on a braille cell that doesn't exist
	 * 
	 * @param commands list of commands that make up the scenario
	 * @param numCells number of initialized cells
	 * @return true if no commands call for cells that don't exist
	 * 			false otherwise
	 */
	public static boolean checkCellNumber(List<PlayerCommand> commands, int numCells) {
		...
		for (PlayerCommand pc : commands) {
			...
			if(pc instanceof CellCharCommand ||pc instanceof CellLowerCommand || pc instanceof CellRaiseCommand || pc instanceof SetPinsCommand || pc instanceof ClearCellCommand) {
				//If they access a cell that doesn't exist, break and return false
				if(Integer.parseInt(pc.getCurrentValue().split("")[0]) >= numCells || Integer.parseInt(pc.getCurrentValue().split("")[0]) < 0) {
					errorFree = false;
					break;
				}
			}
			
			//If a command tries to display a string that's too long, break and return false
	...
	public static boolean checkUserInput(List<PlayerCommand> commands) {
		...
		for(int i = commands.size()-1; i >=0 ; i--) {
			//If a UserInput command has not been seen yet, iterate until one is found
			if(!seenUserInput) {
			...
			//if a UserInput command has been seen, iterate until a SkipButton or RepeatButton command is seen
